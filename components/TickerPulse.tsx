...
const TickerPulse: React.FC<TickerPulseProps> = ({ ticker }) => {
  // Generate consistent mock data based on the ticker string to avoid hydration mismatch or jumping numbers
  // In a real app, this would fetch real historical data
  const { chartData, changePercent, isPositive } = useMemo(() => {
    // Simple seeded random-like generator based on ticker chars to keep it consistent per ticker
    ...
    const points = [];
    let value = 100 + (random() * 100); // Start price between 100-200
  ...
  const points = chartData.map((d, i) => {
    ...
    // Add padding to top/bottom so stroke doesn't clip
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 hover:shadow-sm transition-shadow group cursor-pointer"
